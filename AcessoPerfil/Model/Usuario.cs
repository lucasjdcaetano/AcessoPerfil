...
namespace Model
{
    public class Usuario
    {
        ...
        public static Model.Usuario BuscarUsuario(int id)
        {
            ...
            try
            {
                Model.Usuario usuario = (from u in db.Usuarios where u.Id == id select u).First();
                return usuario;
            }catch
            {
        ...
        {
            Database db = new Database();
            List<Model.Usuario> usuarios = (from u in db.Usuarios select u).ToList();
            return usuarios;
        }
        public static Model.Usuario BuscarPorEmail(string email)
        {
            try {
                Database db = new Database();
                Model.Usuario usuario = (from u in db.Usuarios where u.Email == email select u).First();
                return usuario;
            }catch 
            {
                throw new System.Exception("Usuário não encontrado");
