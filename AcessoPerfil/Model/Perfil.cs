...
namespace Model
{
    ...
    public class Perfil
    {
        ...
        public static Perfil BuscarPerfil(int id)
        {
            ...
            try
            {
                Perfil perfil = (from p in db.Perfis join u in db.Usuarios on p.UsuarioId equals u.Id where p.Id == id select p).First();
                return perfil;
            }catch (Exception)
            {
        ...
        public static Perfil BuscarPerfilPorUsuario(int UsuarioId) 
        {
            ...
            try
            {
                Perfil perfil = (from p in db.Perfis where p.UsuarioId == UsuarioId select p).First();
                return perfil;
            }catch (Exception)
            {
                return null;
